...
---@class custom_radio : druid.base_component
local M = component.create("custom_radio")
...
function M:init(template, state, callback)
  ...
  self.radio = self.druid:new_checkbox(SCHEME.FRONT, callback, SCHEME.INPUT, state)
  self.radio.button:set_style(styles)
...
function M:set_state(state, is_silent, is_instant)
  return self.radio:set_state(state, is_silent, is_instant)
end
